...
async function PageNominee({ slug }: any) {
    ...
    const data:any = await Promise.all(dt?.documents?.map(async (row: any) => {
          ...
          const isDuplicate = dups.has(row?.aspirant_regno?.toLowerCase()) ? true : false;
          if(!isDuplicate) dups.add(row?.aspirant_regno.toLowerCase())
          return { ...row, position, session, isDuplicate }
    }))
        ...
        <table className="w-full border-separate border-spacing-0 border border-blue-900/30 rounded text-[0.83rem] text-blue-900/80 font-medium">
            ...
            { data?.map((row:any,i:React.Key) => {
                ...
                <tr key={row?.serial} className="grid grid-cols-1 md:grid-cols-5 text-left ">
                    <td className={row?.isDuplicate ? `px-6 py-3 grid md:grid-cols-1 gap-y-2 md:border-b border-blue-900/10 bg-red-50/70`: `px-6 py-3 grid md:grid-cols-1 gap-y-2 md:border-b border-blue-900/10`}>
                        ...
                        <span className="ml-3 md:m-0 font-bold text-sm tracking-wide">
                            ...
                            { row.isDuplicate && <span className="px-1.5 py-0.5 my-2 rounded block text-xs font-medium italic"> -- duplicate <br/><br/>( Check update date for newer version )</span> }
                            {/* {JSON.stringify(row)} */}
                    ...
                    <td className="px-6 py-3 grid md:grid-cols-1 md:border-b border-blue-900/10">
                        <span className="md:hidden py-0.5 px-3 rounded bg-blue-900/5 font-bold">POSITION</span>
                        <span className="ml-3 md:m-0 font-semibold">
                            { row?.position?.title?.toUpperCase() }
                            <span className="ml-3 md:m-0 text-xs italic font-medium block">-- Updated: { moment(row?.$updatedAt).format('ddd, MMM DD') }</span>
                        </span>
                    </td>
                    <td className="px-6 py-3 grid md:grid-cols-1 gap-y-2 md:border-b border-blue-900/10">
