'use client'
import { fetchSession } from '@/utils/serverApi';
...
async function UserNav({ session }: Props) {
  ...
  return (
    <aside className="w-full md:m-0 md:w-48 print:hidden">
        ...
        <Link href={`/user/${session?.user?.serial}/application`} className={ pathname == `/user/${session?.user?.serial}/application` ? `p-2 px-6 md:py-4 font-semibold text-white bg-[#153B50]` : `p-2 px-6 md:py-4 font-semibold bg-slate-50`}>APPLICATION FORM</Link>
        <Link href={`/user/${session?.user?.serial}/printout`} className={ pathname == `/user/${session?.user?.serial}/printout` ? `p-2 px-6 md:py-4 font-semibold text-white bg-[#153B50]` : `p-2 px-6 md:py-4 font-semibold bg-slate-50`}>APPLICATION PRINT</Link>
        ...
        <Link href={`/user/${session?.user?.serial}/smsfly`} className={ pathname == `/user/${session?.user?.serial}/smsfly` ? `p-2 px-6 md:py-4 font-semibold text-white bg-[#153B50]` : `p-2 px-6 md:py-4 font-semibold bg-slate-50`}>SMS-FLY SYSTEM <sup>&reg;</sup></Link> 
        </nav> 
    </aside>
  )
}
